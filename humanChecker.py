...
def lineConnected(situation, occupied, attachments):    
    ...
    if rowCheck[0]:
        ...
        while numColumn != len(columns)-1:
            # when numColumn == len(columns) this loop ends
        ...
        while numRow != len(rows)-1:
            # once numRow == len(rows) this loop end
...
def attached(situation, occupied, attachments):
    ''' Checks if the placement of letters are valid to rules '''
...
def getMainCombo(isRowCombo, occupied, situation):
    ''' Get the spots to get main combo'''
    spot = situation[0]
    locations = [spot]
    locator = spot
    if isRowCombo:
...
def getSideCombo(isRowCombo, spot, occupied):
    ''' Get the spots to get side combo'''
    locations = [spot]
    locator = spot
    if isRowCombo:
